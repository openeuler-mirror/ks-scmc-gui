/**
 * @file          /ks-scmc-gui/src/pages/container/security-configuration/security-list-tab.cpp
 * @brief         
...
void SecurityListTab::initUI()
{
    ...
    textBrowser->setHtml("<div id='u1489_text' class='text '>"
                         ...
                         "<p style='font-size:12px;'><span style='font-weight:400;'>填写内容为路径，精确到进程名称；</span></p>"
                         "<p style='font-size:12px;'><span style='font-weight:400;'>当失去光标时，进行判断路径格式是否正确；windows ： 可以使用单斜杠和双斜杠 \\ ，都没有问题、Linux： 认准 /；</span></p>"
                         "<p style='font-size:12px;'><span style='font-weight:400;'>错误信息提示在组件下，提示格式为“进程路径有误，请重新填写”</span></p>"
                         "</div>");
