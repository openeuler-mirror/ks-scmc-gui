      ...
      m_timer(nullptr)
{
    ...
    // TODO:以后在多处调用的maskwidget情况下，在updateInfo中调用该函数
    // setMaskParent(this);
...
void ContainerListPage::onBtnRestart()
{
    ...
    {
        setBusy(true);
        Container::getInstance().restartContainer(m_objId, ids);
    }
}
...
void ContainerListPage::getContainerList()
{
    ...
    if (m_nodeId < 0)
    {
        Container::getInstance().listContainer(m_objId, vecNodeId, true);  // 获取所有容器
    }
    else
    {
        KLOG_DEBUG() << "Get container list of node " << m_nodeId;
        vecNodeId.push_back(m_nodeId);
        Container::getInstance().listContainer(m_objId, vecNodeId, true);  // 获取某节点下的容器
    }
}
